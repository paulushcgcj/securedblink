...
def _parse_yaml_file(file_path: str) -> ConnectionConfig:
    ...
    def get_nested(data: dict[str, Any], keys: list[str]) -> str | None:
        current = data
        for key in keys:
    ...
    if "spring" in data and isinstance(data["spring"], dict):
        ...
        if isinstance(ds, dict):
            if not config.jdbc_url:
                config.jdbc_url = (
                    ds.get("url") or ds.get("jdbc-url") or ds.get("jdbcUrl")
                )
            if not config.username:
    ...
    if not config.jdbc_url:
        config.jdbc_url = (
            data.get("url") or data.get("jdbc_url") or data.get("database_url")
        )
    if not config.username:
