...
def _is_backend_secure() -> bool:
    ...
    try:
        ...
        if "linux" in type(backend).__module__.lower():
            ...
            if "SecretService" in backend_name or "secretstorage" in type(backend).__module__:
                return True
...
def verify_secure_backend() -> None:
    ...
    if not _is_backend_secure():
        ...
        platform_help = help_messages.get(system, "Check your system's credential manager.")
...
class VaultStore:
    ...
    def set(
        ...
        if not overwrite and self._index.get(alias):
            raise VaultStoreError(f"Alias '{alias}' already exists. Use overwrite=True to replace.")
    ...
    def get_metadata(self, alias: str) -> Optional[dict[str, Any]]:
