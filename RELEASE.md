...
1. Log in to [pypi.org](https://pypi.org) and go to
   **Your projects → [project name] → Publishing**.
...
Current version is in `pyproject.toml` → `[project] version`.

### Step 4 — Bump the version

Edit `pyproject.toml` and set the new version string, then commit:
